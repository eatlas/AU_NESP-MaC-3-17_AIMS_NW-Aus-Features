...
3. **Inclusion of Fake (Open Water and Coastline) Locations to Reduce Sampling Bias**:
   - To reduce sampling bias (where validators may expect a feature at every validation location), a
     proportion of the validation locations are intentionally placed in open water or along coastline
     where no mapped reef feature exists.
   - These "fake" locations are generated by:
     - Creating a mask by dissolving all mapped reef features and coastline (land) into a single geometry,
       then buffering the combined geometry by ~5 km. This ensures fake locations are sampled in open water
...
def create_fake_location_mask(features, coastline, regions):
    """
    Create a mask polygon for generating fake features:
    - Dissolve all features and regions into single geometries
    - Clip coastline by dissolved regions (so only coastline within regions is included)
    - Union: (dissolved reefs) + (clipped coastline)
    - Simplify (0.001 deg)
    ...
    dissolved_features = features_4326.dissolve()
    dissolved_features_geom = dissolved_features.geometry.union_all() if hasattr(dissolved_features.geometry, "union_all") else dissolved_features.geometry.unary_union

    # Dissolve all regions into a single geometry
    print("Dissolving regions into a single geometry.")
    dissolved_regions = regions_4326.dissolve()
    dissolved_regions_geom = dissolved_regions.geometry.union_all() if hasattr(dissolved_regions.geometry, "union_all") else dissolved_regions.geometry.unary_union

    # Dissolve all coastline into a single geometry
    print("Dissolving coastline into a single geometry.")
    dissolved_coastline = coastline_4326.dissolve()
    dissolved_coastline_geom = dissolved_coastline.geometry.union_all() if hasattr(dissolved_coastline.geometry, "union_all") else dissolved_coastline.geometry.unary_union

    # Clip coastline by dissolved regions
    print("Clipping coastline by dissolved regions.")
    clipped_coastline_geom = dissolved_coastline_geom.intersection(dissolved_regions_geom)

    # Union reef and (clipped) coastline
    print("Union of reef and clipped coastline geometries.")
    combined_geom = dissolved_features_geom.union(clipped_coastline_geom)

    # Simplify
    print("Simplifying combined geometry.")
    simplified = combined_geom.simplify(0.001)
    # Buffer by 0.05 degrees
    print("Buffering.")
    buffered = simplified.buffer(0.05)
    print("Removing land.")
    # Remove land from buffered area (difference)
    mask = buffered.difference(dissolved_coastline_geom)
    # Simplify again
    mask = mask.simplify(0.001)
    print("Fake location mask created (buffered around both reef and land).")

    # Save fake location mask as shapefile for review
    mask_gdf = gpd.GeoDataFrame(geometry=[mask], crs="EPSG:4326")
...
def create_batch_datasets(features_by_region, batch_num, used_features_indices, coastline, fake_location_mask, crs, features_per_region):
...
def save_validation_datasets(centroids, extents, boundary_points, batch_num, validator, fake_centroids=None):
    """
...
def main():
    ...
    # Load data
    regions, features, full_coastline, simp_coastline = load_data()
    # Reproject all to EPSG:4326 for mask/fake feature logic
    features = features.to_crs("EPSG:4326")
    full_coastline = full_coastline.to_crs("EPSG:4326")
    ...
    for i, validator in enumerate(VALIDATORS):
        ...
        features_per_region = FEATURES_PER_REGION[i]
        used_features_indices = {}
        for batch in range(1, num_batches + 1):
            print(f"\nProcessing batch {batch} of {num_batches} for validator {validator}")
            centroids, extents, boundary_points, fake_centroids = create_batch_datasets(
                features_by_region, batch, used_features_indices, full_coastline, fake_location_mask, features.crs, features_per_region
            )
            save_validation_datasets(centroids, extents, boundary_points, batch, validator, fake_centroids)
